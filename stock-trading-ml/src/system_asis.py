...
class MassiveClient:
    ...
    def bars(self, ticker: str, from_dt: datetime, to_dt: datetime) -> list[dict]:
        from_utc = from_dt + timedelta(hours=ET_OFFSET_H)
        to_utc   = to_dt   + timedelta(hours=ET_OFFSET_H)
        from_ms  = int(from_utc.replace(tzinfo=timezone.utc).timestamp() * 1000)
        to_ms    = int(to_utc.replace(tzinfo=timezone.utc).timestamp() * 1000)
    ...
    def price(self, ticker: str) -> tuple[float, str]:
        ...
        for s in snaps:
            ...
            if lt and getattr(lt, "price", None):
                ...
                if ts_ms:
                    ts_et = (datetime.fromtimestamp(ts_ms / 1000, tz=timezone.utc)
...
def _trading_dates(con) -> list[str]:
    rows = con.execute(f"""
        SELECT DISTINCT
            CAST(epoch_ms(CAST(window_start / 1000000 AS BIGINT) - {ET_OFFSET_H * 3600000}) AS DATE) AS d
    ...
    def _fetch(ticker: str) -> dict | None:
        try:
            ...
            return {
                ...
                "pm_pct": 0.0, "pm_momentum": 0.0, "pm_vol_surge": 1.0,
